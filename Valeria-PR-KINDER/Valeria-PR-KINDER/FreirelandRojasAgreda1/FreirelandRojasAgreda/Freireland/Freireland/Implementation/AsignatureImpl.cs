...
namespace Freireland.Implementation
{
    public class AsignatureImpl : BaseImpl, IAsignature
    {
        ...
        public List<Grade> ObtenerDatosGrados()
        {
            query = @"SELECT id, nameGrade 
                FROM Grade ;";
            SqlCommand command = CreateBasicCommand(query);
            DataTable dataTable = ExecuteDataTableCommand(command);

            List<Grade> grados = new List<Grade>();
            foreach (DataRow row in dataTable.Rows)
            {
                byte id = Convert.ToByte(row["id"]);
                string nameGrade = row["nameGrade"].ToString();

                Grade grado = new Grade(id, nameGrade);
                grados.Add(grado);
            }
        ...
        {
            Asignaturee a = null;
            query = @"SELECT A.id, A.nameAsignature, A.codeAsignature , Ar.id AS areaId,G.id,G.nameGrade, Ar.areaName,
            ...
            INNER JOIN Grade AS G ON A.idGrade = G.id
