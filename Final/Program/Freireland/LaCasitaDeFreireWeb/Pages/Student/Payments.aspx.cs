        ...
        private void Select(int id)
        {
            ...
            try
            {
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    table.Append("<td>" + dt.Rows[pos][7].ToString() + "</td>");
                    table.Append("<td>" + dt.Rows[pos][8].ToString() + "</td>");
                    if (dt.Rows[pos][1].ToString() == "NO PAGADA")
                    {
                    ...
                    else
                    {
                        ...
                        //table.Append("<button type='button' class='btn btn-secondary' data-bs-toggle='modal' data-bs-target='#DeleteModal" + dr[0] + " ' disabled>Pagado</button>"+ "<a class='btn btn-outline-info' href='Detail.aspx?" + dr[0] +"'>Detalle</a>" + "<button type='button' class='btn btn-outline-danger' data-bs-toggle='modal' data-bs-target='#DeleteModalA" + dr[0] + " '>Revertir</button></div></div></td>");
                ...
                tableP.Append("<table class='table'>");
                tableP.Append("<thead>" +
                            "<th class='text-center'>ESTADO</th>" +
                            "<th>Monto</th>" +
                            "<th>Fecha Limite de Pago</th>" +
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    {
                        tableP.Append("<tr>");
                        tableP.Append("<td class='text-center bg-danger text-white'>" + dt.Rows[pos][1].ToString() + "</td>");

                    }
                    else
                    {
                        tableP.Append("<tr bg-success>");
                        tableP.Append("<td class='text-center bg-success text-white'>" + dt.Rows[pos][1].ToString() + "</td>");

                    }
                    tableP.Append("<td>" + dt.Rows[pos][2].ToString() + "</td>");
                    tableP.Append("<td>" + dt.Rows[pos][3].ToString() + "</td>");
                    tableP.Append("<td>" + dt.Rows[pos][4].ToString() + "</td>");
                    tableP.Append("<td>" + dt.Rows[pos][5].ToString() + "</td>");
                    tableP.Append("<td>" + dt.Rows[pos][6].ToString() + "</td>");
                    tableP.Append("<td>" + dt.Rows[pos][7].ToString() + "</td>");
                    tableP.Append("<td>" + dt.Rows[pos][8].ToString() + "</td>");
