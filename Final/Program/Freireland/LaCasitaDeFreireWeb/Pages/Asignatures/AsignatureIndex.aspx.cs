        ...
        void Select(string search)
        {
            try
            {
                ...
                table.Append("<thead>" +
                            ...
                            //"<th>Ci</th>" +
                            "<th class='text-center'><div class='d-grid gap-2'>" + "<a type='button' href='AsignatureCreate.aspx' class='btn btn-success'>+ Nueva Materia</a>" + "</div></th>" +
                            "</thead>");
                int pos;
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    table.Append("<td class='text-center'><div class='d-grid gap-2'><div class='btn-group'>" + "<a class='btn btn-outline-warning' href='AsignaturaUpdate.aspx?ID=" + dr[0] + "'>Editar</a>");
            ...
            try
            {
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    table.Append("<div class='modal fade'  id='DeleteModal" + dr[0] + "' tabindex='-1' aria-labelledby='DeleteModal" + dr[0] + "' aria-hidden='true'>" +
                        ...
                        "<div class='modal-content'>" +
                        "<div class='modal-header bg-danger'>" +
                        "<h1 class='modal-title fs-5 text-light'  id='DeleteModalLabel'>Eliminar Materia</h1>" +
                        "<button type='button' class='btn-close' data-bs-dismiss='modal' aria-label='Cerrar'></button>" +
                        "</div><div class='modal-body'> Esta seguro/a de eliminar esta Materia? | " + dr[1] + "</div>" +
                        "<div class='modal-footer'>" +
                        "<button type='button' class='btn btn-outline-secondary' data-bs-dismiss='modal'>Cancelar</button>" +
