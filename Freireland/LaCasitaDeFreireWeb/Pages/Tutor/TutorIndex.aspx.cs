        ...
        void SelectSearch(string search) 
        {
            try
            {
                ...
                table.Append("<thead>" +
                            ...
                            "<th>Cellphone</th>" +
                            "<th>WorkPlace</th>" +
                            "<th class='text-center'>" + "<a type='button' href='TutorCreate.aspx' class='btn btn-success'>+ Nuevo Tutor</a>" + "</th>" +
                            "</thead>");
                int pos;
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    table.Append("<td class='text-center'>" + "<a class='btn btn-outline-success' href='StudentCreate.aspx?ID=" + dr[0] + "'>+ Hijo</a>" + "<a class='btn btn-outline-warning' href='TutorUpdate.aspx?ID=" + dr[0] + "'>Editar</a>");
        ...
        void Select()
        {
            try
            {
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    table.Append("<td>" + "<a class='btn btn-outline-success' href='#'>" + dr[4] + "</a>" + "</td>");
                    table.Append("<td>" + dt.Rows[pos][5].ToString() + "</td>");
                    table.Append("<td class='text-center'>" + "<a class='btn btn-outline-success' href='" + ResolveUrl("~/Pages/Student/StudentCreate.aspx") + "?ID=" + dr[0] + "'>+ Hijo</a>" + "<a class='btn btn-outline-warning' href='TutorUpdate.aspx?ID=" + dr[0] + "'>Editar</a>");
                    table.Append("<button type='button' class='btn btn-outline-danger' data-bs-toggle='modal' data-bs-target='#DeleteModal" + dr[0] + "'>Eliminar</button>" + "</td>");
                    table.Append("</tr>");
                    table.Append("<div class='modal fade'  id='DeleteModal" + dr[0] + "' tabindex='-1' aria-labelledby='DeleteModal" + dr[0] + "' aria-hidden='true'>" +
                        "<div class='modal-dialog'>" +
