        ...
        void Select(string search)
        {
            try
            {
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    table.Append("<td class='text-center'><div class='button-group'>" + "<a class='btn btn-outline-success' href='StudentUpdate.aspx?ID=" + dr[0] + "'>Pagos</a>");
                    table.Append("<a class='btn btn-outline-warning' href='StudentUpdate.aspx?ID=" + dr[0] + "'>Editar</a>");
                    table.Append("<button type='button' class='btn btn-outline-danger' data-bs-toggle='modal' data-bs-target='#DeleteModal" + dr[0] + "'>Eliminar</button>" + "</div></td>");
                    table.Append("</tr>");
        ...
        void Select()
        {
            try
            {
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    // table.Append("<td>" + dt.Rows[pos][3].ToString() + "</td>");
                    // table.Append("<td>" + dt.Rows[pos][4].ToString() + "</td>");
                    table.Append("<td class='text-center'><div class='button-group'>" + "<a class='btn btn-outline-success' href='Payments.aspx?ID=" + dr[0] + "'>Pagos</a>");
                    table.Append("<a class='btn btn-outline-warning' href='StudentUpdate.aspx?ID=" + dr[0] + "'>Editar</a>");
                    table.Append("<button type='button' class='btn btn-outline-danger' data-bs-toggle='modal' data-bs-target='#DeleteModal" + dr[0] + "'>Eliminar</button>" + "</div></td>");
                    table.Append("</tr>");
                    table.Append("<div class='modal fade'  id='DeleteModal" + dr[0] + "' tabindex='-1' aria-labelledby='DeleteModal" + dr[0] + "' aria-hidden='true'>" +
